...
// ─────────────────────────────────────────────────────────────────────────────
// Portable Mode Detection (Isolate data within project)
// ─────────────────────────────────────────────────────────────────────────────
const isDev = !app.isPackaged || process.env.NODE_ENV === 'development' || process.env.DEBUG === 'true';
const projectRoot = isDev
  ? resolve(__dirname, '../../../../../')
  : resolve(dirname(app.getPath('exe')), '../..');

if (existsSync(join(projectRoot, '.auto-claude'))) {
  const localUserData = join(projectRoot, '.auto-claude-ui');
  if (!existsSync(localUserData)) {
    mkdirSync(localUserData, { recursive: true });
  }
  app.setPath('userData', localUserData);
  console.log(`[main] Portable Mode detected: ${projectRoot}`);
  console.log(`[main] Using local userData: ${localUserData}`);
  (global as any).PORTABLE_PROJECT_ROOT = projectRoot;
}
...
app.whenReady().then(async () => {
  ...
  // Auto-register project in portable mode
  const portableProjectRoot = (global as any).PORTABLE_PROJECT_ROOT;
  if (portableProjectRoot) {
    // Use dynamic import to avoid circular dependencies during startup
    // @ts-ignore - Dynamic import of local module
    const { projectStore } = await import('./project-store');
    console.log(`[main] Auto-registering portable project: ${portableProjectRoot}`);
    const project = projectStore.addProject(portableProjectRoot);

    // Also update tab state to make this project active if none is active

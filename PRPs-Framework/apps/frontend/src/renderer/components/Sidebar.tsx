...
export function Sidebar({
  ...
  const visibleNavItems = useMemo(() => {
    ...
    // Filter out AI features if dashboardOnlyMode is enabled
    if (settings.dashboardOnlyMode) {
      const aiFeatures = ['ideation', 'roadmap', 'context', 'agent-tools', 'insights'];
      return items.filter(item => !aiFeatures.includes(item.id));
    }

    return items;
  }, [githubEnabled, gitlabEnabled, settings.dashboardOnlyMode]);
    ...
    <TooltipProvider>
      <div className={cn(
        ...
        <div className={cn("space-y-3 transition-all duration-300", isCollapsed ? "p-2" : "p-4")}>
          ...
          {!isCollapsed && selectedProject && !selectedProject.autoBuildPath && !settings.dashboardOnlyMode && (

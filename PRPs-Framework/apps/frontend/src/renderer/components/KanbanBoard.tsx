...
export function KanbanBoard({ tasks, onTaskClick, onNewTaskClick, onRefresh, isRefreshing }: KanbanBoardProps) {
  ...
  const dashboardOnlyMode = useSettingsStore((state) => state.settings.dashboardOnlyMode);
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {/* Kanban columns */}
      <DndContext
        sensors={dashboardOnlyMode ? [] : sensors}
        collisionDetection={closestCorners}
        onDragStart={dashboardOnlyMode ? undefined : handleDragStart}
        onDragOver={dashboardOnlyMode ? undefined : handleDragOver}
        onDragEnd={dashboardOnlyMode ? undefined : handleDragEnd}
      >
        <div className="flex flex-1 gap-4 overflow-x-auto p-6">
          {TASK_STATUS_COLUMNS.map((status) => (
            <DroppableColumn
              ...
              onStatusChange={dashboardOnlyMode ? undefined : handleStatusChange}
              isOver={overColumnId === status}
              onAddClick={!dashboardOnlyMode && status === 'backlog' ? onNewTaskClick : undefined}
              onQueueAll={!dashboardOnlyMode && status === 'backlog' ? handleQueueAll : undefined}
              onQueueSettings={!dashboardOnlyMode && status === 'queue' ? () => {
                // Only open modal if we have a valid projectId
                if (!projectId) return;
                queueSettingsProjectIdRef.current = projectId;
                setShowQueueSettings(true);
              } : undefined}
              onArchiveAll={!dashboardOnlyMode && status === 'done' ? handleArchiveAll : undefined}
              ...
              onToggleArchived={!dashboardOnlyMode && status === 'done' ? toggleShowArchived : undefined}
              selectedTaskIds={dashboardOnlyMode ? undefined : selectedTaskIds}
              onSelectAll={dashboardOnlyMode ? undefined : () => selectAllTasks(status)}
              onDeselectAll={dashboardOnlyMode ? undefined : deselectAllTasks}
              onToggleSelect={dashboardOnlyMode ? undefined : toggleTaskSelection}
              ...
              onResizeStart={dashboardOnlyMode ? undefined : (startX) => handleResizeStart(status, startX)}
              onResizeEnd={dashboardOnlyMode ? undefined : handleResizeEnd}
              isLocked={columnPreferences?.[status]?.isLocked}
              onToggleLocked={dashboardOnlyMode ? undefined : () => handleToggleColumnLocked(status)}
            />
          ))}
        </div>

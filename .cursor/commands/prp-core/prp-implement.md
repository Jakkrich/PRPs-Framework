...
**Plan Path**: `.auto-claude/specs/{ID}/implementation_plan.json`

```python
sys.path.append("PRPs-Framework")
from apps.tools.json_executor import JsonExecutor
executor = JsonExecutor(".auto-claude/specs/{ID}/implementation_plan.json")
```
...
**For each task in the JSON plan's pending list:**

### 3.1 Get Next Task

```python
phase, task_id, task_data = executor.get_next_pending_task()
print(f"Executing {task_id}: {task_data['description']}")
```

### 3.2 Implement

1. Read the `task_data` descriptions.
2. Implement changes.
3. Validate.
...
### 3.4 Update Plan Status

Run `python PRPs-Framework/apps/tools/json_executor.py complete {plan_path} {task_id}` to mark it as `completed`.
...
**CRITICAL: THIS PHASE IS MANDATORY. DO NOT SKIP.**
**You MUST create an implementation report to document the work done.**
**Without this report, the work is considered INCOMPLETE.**

### 5.1 Create Report Directory

```bash
mkdir -p .auto-claude/reports
```

### 5.2 Generate Report

**Path**: `.auto-claude/reports/{plan-name}-report.md`
...
```bash
mkdir -p .auto-claude/plans/completed
mv $ARGUMENTS .auto-claude/plans/completed/
```

**PHASE_5_CHECKPOINT:**

- [ ] Report created at `.auto-claude/reports/`
...
- Report: `.auto-claude/reports/{name}-report.md`
- Plan archived to: `.auto-claude/plans/completed/`

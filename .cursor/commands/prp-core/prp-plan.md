...
## Phase 6: GENERATE - Implementation Plan (JSON + Markdown)

**OUTPUT_FOLDER**: `.auto-claude/specs/{ID}-{slug}/`
**JSON_FILE**: `implementation_plan.json`
**MARKDOWN_FILE**: `plan.md`

**Process**:
1. **Initialize JSON**: Run `python PRPs-Framework/apps/tools/json_planner.py create .auto-claude/specs/{ID}-{slug}/spec.md`
2. **Populate JSON**: Update the `phases` and `subtasks` in `implementation_plan.json` based on the research.
   - Use `python PRPs-Framework/apps/tools/json_planner.py add-phase ...` or edit directly if robust.
3. **Generate Markdown View**: Create `plan.md` as a human-readable summary.
...
<output>
**OUTPUT_FILE**: `.auto-claude/plans/{kebab-case-feature-name}.plan.md`
...
**File**: `.auto-claude/plans/{feature-name}.plan.md`
...
To start: `git worktree add -b phase-{X} ../project-phase-{X} && cd ../project-phase-{X} && /prp-core/prp-plan {prd-path}`
...
**Next Step**: To execute, run: `/prp-core/prp-implement .auto-claude/plans/{feature-name}.plan.md`
````
